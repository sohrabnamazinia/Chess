...
public class Board
{
    ...
    public boolean move(String from, String to, String color)
    {
        ...
        if( fromArray.length == 2 && toArray.length == 2 )
        {
            ...
            if( fromArray[0] >= '1' && fromArray[0] <= '8' && fromArray[1] >= 'A' && fromArray[1] <= 'H' )
            {
                // Check from bound
    ...
    public boolean canGo(char[] fromArray, char []toArray, String color)
    {
        ...
        if( found != null && found.getColor() == booleanColor && !toBusy )
        {
            ...
            else
            {
                if( toArray[0] >= '1' && toArray[0] <= '8' && toArray[1] >= 'A' && toArray[1] <= 'H' )
                {
                    if( found.canMove(toArray[1], toArray[0] - '0') )
                    {
                        // Check if move is valid
    ...
    public boolean canAttack(Piece p, char x, int y, String color)
    {
        ...
        if( found.getColor() == booleanColor && !toBusy )
        {
            if( kingCheck(selector, enemy) && !(found instanceof King) )
            {
                return false;
